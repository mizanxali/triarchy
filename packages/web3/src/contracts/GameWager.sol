...
contract GameWager {
    ...
    function getGame(string memory gameCode) external view returns (Game memory) {
    ...
    function createGame(string memory gameCode) external payable {
        ...
        require(games[gameCode].player1 == address(0), "Game code already exists");
    ...
    function joinGame(string memory gameCode) external payable gameExists(gameCode) {
        Game storage game = games[gameCode];
        
        require(game.isActive, "Game is not active");
        require(game.player2 == address(0), "Game already has two players");
        require(msg.sender != game.player1, "Cannot join your own game");
        require(msg.value == game.wagerAmount, "Must match the exact wager amount");
    ...
    function completeGame(string memory gameCode, address winner) external onlyOwner gameExists(gameCode) {
        Game storage game = games[gameCode];
        ...
        uint256 prizeMoney = game.wagerAmount * 2;
        payable(winner).transfer(prizeMoney);
        
        emit GameComplete(gameCode, winner, prizeMoney);
    }
    
    function cancelGame(string memory gameCode) external onlyOwner gameExists(gameCode) {
        Game storage game = games[gameCode];
        require(game.isActive, "Game is not active");
